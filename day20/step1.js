...
a.sample = 10;
console.log("상수a sample의 값 = "+a.sample);
console.log("상수a 의 타입 = "+  typeof a);
console.log (Array.isArray(a) ) // is~~~ : ~이다? 확인하는 //Array.isArray( 상수/ 변수 ) : 해당 상수/변수에 자료가 배열 자료형인지 확인
...
f.sample = 10;
    console.log( f );       // Number { 273, sample : 10 }  // 객체는 키와 값으로 이뤄진 속성
console.log(f.valueOf());   // 273  객체.valueOf();          // 속성이 아닌 값 추출.

const g = Number(273);      // [ 객체X ]
    console.log(g);
...
console.log(Number.prototype.sample)

const j = 100;
console.log(i.sample);  // 가능.???? 10
const j2 = new Number( 275 );
...
// p.265
const h = '안녕하세요';
console.log( h.indexOf('안녕') );       // 문자열내 안녕이 존재하면 앞자리 인덱스로 추출.
...
// js가 편의성으로 제공하면 좋은대 없으니까 내가 만들자.
