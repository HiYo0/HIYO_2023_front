...
document.addEventListener('DOMContentLoaded',function(){
    ...
    textarea.addEventListener('keyup' , function(e){
        ...
        console.log(e.code);     // 입력한키의 문자열로
        console.log(e.keyCode);  // 입력한키 숫자로 표시
        console.log(e.altKey);      // [조합키] alt 눌렀는지
        console.log(e.ctrlKey);     // [조합키] ctlr 눌렀는지
    ...
    // 3. 별 위치를 이동하는 이벤트
            // 3-1 키보드 방향키의 키번호확인
        const left = 37;    const up = 38;  const right = 39;   const donw = 40;
            // 3-1 body(전체)에서 키를 눌렀을때 이벤트 발생
        document.body.addEventListener('keydown',function(e){
            console.log(e);

            if(e.keyCode == left){ x-- ;}       // 왼쪽방향키 눌렀을때. x 1 감소
            else if(e.keyCode == up){y--;}      // 위 방향키를 눌렀을때. y 1 감소
            else if(e.keyCode == right){x++;}   // 오른쪽 방향키를 눌렀을때. x 1 증가
            else if(e.keyCode == donw){y++;}    // 아래 방향키를 눌렀을때. y 1 증가
            else{ }
