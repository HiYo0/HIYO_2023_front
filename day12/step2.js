    ...
    배열
        ...
        2. 배열 요소에 접근
            - 인덱스 란 ??? 배열내 요소들이 저장된 순서번호
                0부터 시작  ( 주의할점 length/size 는 1붵 시작 )
            - 형태
                변수명[인덱스]
        3. 배열 요소 개수 확인
            배열.       
        4. 배열 뒷부분에 요소 추가하기
            ...
            - 중간(특정위치) 삽입
            배열.splice( 추가할 요소의 인덱스위치 , 0 , 추가할 요소의 값 )

        5. 배열 요소 제거하기
            배열.splice( 제거할 요소의 인덱스, 제거할 요소의 개수 )
                - 배열.splice( 0 ); : 배열 요소 모두삭제
...
*/
const str = "안녕하세요"
console.log( str[2] );  // gk
console.log( str[ str.length-1] );  // 요 // str[ str.length-1] :  오류발생(없는 인덱스라서)

// p.161
const array = [273, 'string', true, function(){}, {}, [ 273,10 ] ];
        // 가장큰 쇼핑백 = [ 273, ['s','t','r','t','i','n','g'], true, 함수(){}, { 객체 }, 배열 ];
        // 가장큰 쇼핑백 = [ 숫자자료형, 배열자료형, 불자료형, 함수, 객체, 배열자료형 ];
console.log( array ); console.log( array[3] ); console.log( array[5] );
console.log( array[5][0] ); // array[5] => [273,10][0] => 273
...
// ['우유 구매','업무 메일 확인하기','필라테스 수업','저녁 식사 준비하기',empty,empty,'사과먹기' ]
//      0           1                   2               3               4    5       6
...
// itemsA.splice( 0 );     // 삭제 개수 생략시 삭제할 인덱스 뒤로 모두 삭제됨
// console.log( itemsA );
